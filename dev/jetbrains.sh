...
if [[ $IS_WSL -eq 1 ]]; then
	if ! grep -q "ln -sf /mnt/wslg/runtime-dir/wayland-\* \$XDG_RUNTIME_DIR" "$HOME/.bashrc"; then
		log_info "Adding Wayland runtime configuration to .bashrc..."
		echo "$WAYLAND_RUNTIME" >>"$HOME/.bashrc"
	fi
	if ! grep -q "ln -sf /mnt/wslg/runtime-dir/wayland-\* \$XDG_RUNTIME_DIR" "$HOME/.zshrc"; then
		log_info "Adding Wayland runtime configuration to .zshrc..."
		echo "$WAYLAND_RUNTIME" >>"$HOME/.zshrc"
	fi
fi
